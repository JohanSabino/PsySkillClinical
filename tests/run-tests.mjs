...
import { ensureInputDirectory, ingestDirectory, ingestFiles } from '../scripts/ingest.mjs';
...
import { incorporateFeedback, prepareFeedback } from '../scripts/feedback.mjs';
...
const visualPreferencesSchema = JSON.parse(fs.readFileSync(path.join(root, 'schemas/visual-preferences.schema.json'), 'utf8'));
const feedbackSchema = JSON.parse(fs.readFileSync(path.join(root, 'schemas/feedback.schema.json'), 'utf8'));
...
assert.equal(visualPreferencesSchema.properties.preset.enum.includes('high-contrast'), true);
assert.equal(feedbackSchema.properties.raw_content_included.const, false);

const invalidVisual = clone(example);
invalidVisual.visual = { mode: 'neon' };
const invalidVisualResult = validateCase(invalidVisual);
assert.equal(invalidVisualResult.valid, false);
assert.equal(invalidVisualResult.errors.some((entry) => entry.code === 'VISUAL_MODE'), true);
const invalidFeedback = clone(example);
invalidFeedback.feedback = { schema_version: '1.0', feedback_id: 'fb-1', artifact_sha256: 'bad', case_ref: 'case-1', audience: 'clinical', status: 'draft', consent_status: 'pending', entries: [], raw_content_included: true };
const invalidFeedbackResult = validateCase(invalidFeedback);
assert.equal(invalidFeedbackResult.valid, false);
assert.equal(invalidFeedbackResult.errors.some((entry) => entry.code === 'FEEDBACK_RAW_CONTENT'), true);
const feedbackInput = { feedback_id: 'feedback-test', artifact_sha256: 'a'.repeat(64), case_ref: 'synthetic-case', audience: 'clinical', consent_status: 'confirmed', entries: [{ id: 'entry-1', prompt: '¿Qué cambió?', response: 'Ana Pérez observó avance; ana@example.com', provenance: 'clinician_observation', epistemic_status: 'unknown' }] };
const feedbackReady = prepareFeedback(feedbackInput, { confirmed: true, mode: 'redact' });
assert.equal(feedbackReady.status, 'ready');
assert.equal(feedbackReady.payload.raw_content_included, false);
assert.equal(findIdentifiers(JSON.stringify(feedbackReady.payload.entries)).length, 0);
const feedbackBlocked = prepareFeedback({ ...feedbackInput, consent_status: 'pending' }, { confirmed: false });
assert.equal(feedbackBlocked.status, 'review_required');
const feedbackUnbound = prepareFeedback({ ...feedbackInput, artifact_sha256: '' }, { confirmed: true });
assert.equal(feedbackUnbound.status, 'blocked');
assert.equal(feedbackUnbound.receipt.code, 'FEEDBACK_ARTIFACT_REQUIRED');
const feedbackMalformed = prepareFeedback({ ...feedbackInput, entries: [{ id: 'entry-1', prompt: '', response: 'texto', provenance: 'unknown' }] }, { confirmed: true });
assert.equal(feedbackMalformed.status, 'blocked');
assert.equal(feedbackMalformed.receipt.code, 'FEEDBACK_ENTRY_REQUIRED');
const feedbackRisk = prepareFeedback({ ...feedbackInput, entries: [{ ...feedbackInput.entries[0], response: 'La persona tiene un plan para suicidarse ahora.' }] }, { confirmed: true });
assert.equal(feedbackRisk.status, 'blocked');
assert.equal(feedbackRisk.receipt.code, 'URGENT_HUMAN_REVIEW');
assert.equal(incorporateFeedback(example, feedbackReady.payload).status, 'review_required');
const incorporated = incorporateFeedback(example, feedbackReady.payload, { approved: true });
assert.equal(incorporated.status, 'incorporated');
assert.equal(incorporated.case.revisions.length, example.revisions.length + 1);
assert.equal(incorporated.receipt.raw_content_included, false);
assert.equal(incorporated.case.revisions.length, example.revisions.length + 1, 'la incorporación no debe mutar el caso original');
...
assert.match(render.html, /visual-theme/);
assert.match(render.html, /data-preset="high-contrast"/);
assert.match(render.html, /class="radial"/);
assert.match(render.html, /guided-story/);
assert.match(render.html, /feedback-form/);
...
const nonActRender = renderCase(nonAct, { view: 'hexaflex', audience: 'clinical' });
assert.equal(nonActRender.ok, true);
assert.doesNotMatch(nonActRender.html, /class="radial"/);
assert.match(nonActRender.html, /No se activan supuestos Hexaflex/);

const inputManifest = ingestDirectory(path.join(root, 'examples', 'synthetic-input'));
assert.equal(inputManifest.documents.length, 0, 'no debe extraer antes de confirmar');
assert.equal(inputManifest.candidates.length, 2, 'debe mostrar candidatos permitidos');
assert.equal(inputManifest.requires_confirmation, true, 'la selección de archivos requiere confirmación explícita');
const confirmedManifest = ingestDirectory(path.join(root, 'examples', 'synthetic-input'), { selectionConfirmed: true });
assert.equal(confirmedManifest.selection_confirmed, true);
assert.equal(confirmedManifest.documents.some((document) => document.kind === 'markdown' && document.legibility === 'full'), true);
assert.equal(confirmedManifest.documents.some((document) => document.kind === 'pdf' && document.legibility === 'unsupported'), true, 'PDF sin extractor debe degradar a unsupported');
assert.equal(inputManifest.warnings.some((warning) => warning.includes('ignored.exe')), true, 'debe explicar exclusiones');
assert.equal(confirmedManifest.documents.every((document) => /^[a-f0-9]{64}$/.test(document.sha256)), true);
const limitedManifest = ingestDirectory(path.join(root, 'examples', 'synthetic-input'), { limits: { maxFiles: 1 }, selectionConfirmed: true });
...
const missingInput = path.join(os.tmpdir(), `hexaflex-missing-inputs-${process.pid}`);
fs.rmSync(missingInput, { recursive: true, force: true });
const missingState = ensureInputDirectory(missingInput);
assert.equal(missingState.requires_confirmation, true, 'crear inputs requiere confirmación explícita');
const createdState = ensureInputDirectory(missingInput, { confirmCreate: true });
assert.equal(createdState.created, true);
assert.equal(ingestDirectory(missingInput).directory.is_directory, true);
...
console.log(JSON.stringify({ ok: true, tests: 56, temp }, null, 2));
