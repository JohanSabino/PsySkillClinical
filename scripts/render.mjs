...
function hexaflexSection(data, patient, audience = patient ? 'patient' : 'clinical') {
  ...
  const processCards = all.map((process) => `<article ${filterAttrs({ id: `process-${process}`, processes: [process], audience, evidence: 'fact' })}><h3>${esc(processLabels[process])}</h3><p>${patient ? 'Una perspectiva para explorar con curiosidad y flexibilidad.' : 'Lente interrelacionada; no es una puntuación ni una etapa obligatoria.'}</p></article>`).join('');
  const patternCards = patterns.map((pattern) => `<article ${filterAttrs({ id: pattern.id, processes: pattern.processes, audience, evidence: evidenceStatus(pattern) })}><h3>${esc(pattern.title)}</h3><p>${tags(pattern.processes)}</p>${patient ? list(pattern.value_directions?.map((item) => item.text)) : `<details><summary>Ver elementos de formulación</summary><p><strong>Contexto:</strong></p>${list(pattern.context)}<p><strong>Experiencias y conductas:</strong></p>${list([...(pattern.internal_experiences ?? []), ...(pattern.behaviors ?? [])])}<p><strong>Hipótesis funcional:</strong> <span class="tag hyp">revisable</span> ${esc(pattern.function_hypothesis?.text)}</p><p><strong>Efectos a corto plazo:</strong></p>${list(pattern.short_term_effects)}<p><strong>Efectos a largo plazo:</strong></p>${list(pattern.long_term_effects)}</details>`}</article>`).join('');
...
function renderBody(data, view, audience, result) {
  ...
  return `${patient && audience === 'patient' ? '<p class="notice" role="status">Previsualización psicoeducativa: aún no está marcada como material compartible.</p>' : ''}${summarySection(data, patient, audience)}${sections.join('')}`;
}
