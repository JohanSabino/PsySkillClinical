...
import { prepareFeedback } from './feedback.mjs';

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const required = ['SKILL.md', 'agents/openai.yaml', 'schemas/case.schema.json', 'schemas/visual.schema.json', 'schemas/visual-preferences.schema.json', 'schemas/feedback.schema.json', 'schemas/session.schema.json', 'schemas/document.schema.json', 'schemas/privacy.schema.json', 'assets/viewer-template.html', 'examples/synthetic-case.json', 'examples/synthetic-feedback.json', 'references/formulation.md', 'references/session-planning.md', 'references/interventions.md', 'references/safety-privacy.md', 'references/visual-language.md', 'references/document-ingestion.md', 'references/source-policy.md', 'scripts/ingest.mjs', 'scripts/privacy.mjs', 'scripts/feedback.mjs'];
...
try {
  ...
  const ingestion = ingestDirectory(path.join(root, 'examples', 'synthetic-input'), { capabilities, selectionConfirmed: true });
  ...
  const scriptsText = fs.readFileSync(path.join(root, 'scripts', 'ingest.mjs'), 'utf8') + fs.readFileSync(path.join(root, 'scripts', 'privacy.mjs'), 'utf8') + fs.readFileSync(path.join(root, 'scripts', 'feedback.mjs'), 'utf8') + fs.readFileSync(path.join(root, 'scripts', 'render.mjs'), 'utf8');
  ...
  const feedback = prepareFeedback({ feedback_id: 'doctor-feedback', artifact_sha256: 'a'.repeat(64), case_ref: data.meta.case_ref, audience: 'clinical', consent_status: 'confirmed', entries: [{ id: 'entry', prompt: 'Qué cambió', response: 'Nada por ahora.', provenance: 'clinician_observation', epistemic_status: 'unknown' }] }, { confirmed: true, mode: 'redact' });
  checks.push({ name: 'feedback_export', passed: feedback.status === 'ready' && feedback.payload.raw_content_included === false, detail: 'Feedback local listo, minimizado y ligado a hash' });
} catch (error) {
